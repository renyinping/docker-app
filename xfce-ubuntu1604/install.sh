...
build() {
    [ ! -z "$1" ] && local ver=:$1
    docker build --rm --no-cache -t yinping/xfce${ver} .
}

run() {
    [ ! -z "$1" ] && local ver=:$1
    docker run --cap-add SYS_ADMIN --name xfce -p 5901:5901 -p 6901:6901 -e VNC_RESOLUTION=1440x900 -e VNC_PW=vncpassword -d yinping/xfce${ver}
